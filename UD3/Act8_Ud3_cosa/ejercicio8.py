...
import sqlite3 # Librería para la gestión de bases de datos SQLite (para utilizar el QSqlTableModel que alfinal no he utilizado)
...
class mainWindow(QMainWindow):
    def __init__(self):
        ...
        self.insertar_demo()
        # Ahora muestra False ya que la tabla ya no está vacia
        print(self.tabla_vacia())
        # Llamada a la función que carga los contactos al iniciar el programa
    ...
    def eliminar_contacto(self):
        ...
        if respuesta == QMessageBox.Yes:
            # Obtener los datos del contacto que se ha seleccionado en listaContactos
            texto = contactoSel.text()
            partes = texto.split(" | ")
            nombre = partes[0]
            telefono = partes[1]
            email = partes[2]
            ...
            # Creo la consulta 
            consulta = QSqlQuery()
            # Creo el sql que eliminará los datos del contacto con el contenido que se la pasa a la funcion añadir_contacto
            consulta.prepare(
                "DELETE FROM contactos WHERE nombre = ? AND telefono = ? AND email = ?"
                )
            # Añade los valores en orden de cada columna
            consulta.addBindValue(nombre)
            consulta.addBindValue(telefono)
            ...
            if not consulta.exec():
                QMessageBox.warning(
                    self, "Error al eliminar", "No se ha podido eliminar el contacto"
                )
                return
